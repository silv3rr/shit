<?php
/*--------------------------------------------------------------------------*
 *   SHIT:WEB normal / systemd mode cmds
 *--------------------------------------------------------------------------*/
$cmds = array(
    ...
    'glftpd_status'   => $sudo_systemd . 'status glftpd.socket',
    'glftpd_start'    => $sudo_systemd . 'start glftpd.socket',
    'glftpd_stop'     => $sudo_systemd . 'stop glftpd.socket',
    'glftpd_stop'     => $sudo_systemd . 'stop glftpd.socket',
    'glftpd_restart'  => $sudo_systemd . 'restart glftpd.socket',
...
/*--------------------------------------------------------------------------*
 * OLD/UNUNUSED:
 *   'ls'            => '( cd $bot_dir/bot; ls -la )'
 *   'who'           => '( cd $bot_dir/bot; sudo -u shit /usr/bin/whoami)',
